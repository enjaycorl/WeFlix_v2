  ...
  return (
    <div className="flex flex-col min-h-screen">
      <SEO
        ...
        description={
          genre
            ? `Browse and stream ${genre.name} movies free on PirTV. Discover the best ${genre.name.toLowerCase()} films, from classics to new releases.`
            : 'Browse and stream trending movies free on PirTV. Discover the most popular, top-rated, and newest films across all genres.'
        }
        url={`https://www.weflix.app${buildBrowsePath('movie', genreId)}`}
      />
      {/* Sticky context header */}
      <div className="sticky top-0 z-40 backdrop-blur-md bg-[#160e24]/80 border-b border-white/[0.06]">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-white/[0.03] border border-white/[0.08] shadow-sm">
              <BiMoviePlay className="text-brand-500 text-xl" />
            </div>
            <div className="flex flex-col">
              ...
              {genre ? (
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className="px-2 py-0.5 rounded shadow-sm bg-brand-500/10 border border-brand-500/20 text-brand-400 text-[11px] font-bold tracking-wider uppercase leading-none">
                    {genre.name}
                  </span>
                </div>
              ) : (
          ...
          {genreId && (
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-white/[0.07] border border-white/10 text-sm text-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:ring-1 focus:ring-brand-500 cursor-pointer"
              >
                {SORT_OPTIONS.map(opt => (
                  <option key={opt.value} value={opt.value} className="bg-[#160e24]">{opt.label}</option>
                ))}
              </select>
            </div>
          )}
